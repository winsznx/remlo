...
contract ReputationRegistryUpgradeable is OwnableUpgradeable, UUPSUpgradeable {
    ...
    // Upstream used `reinitializer(2) onlyOwner` — upgrade path only. Fresh
    // deploys need the standard `initializer` pattern; see
    // IdentityRegistryUpgradeable.initialize for the full rationale and
    // AGENT_PROGRESS Ship 6.3 for the drift log.
