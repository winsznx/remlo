    ...
    EIP712Upgradeable
{
    ...
    // Upstream's original modifier was `reinitializer(2) onlyOwner` — designed
    // for an on-chain UPGRADE from an already-deployed v1 proxy. On a fresh
    // deploy, `owner()` returns address(0) so `onlyOwner` always fails. We
    // swap to the standard `initializer` pattern and bootstrap Ownable +
    // UUPSUpgradeable with the deployer as owner. Drift from upstream is
    // documented in AGENT_PROGRESS Ship 6.3.
