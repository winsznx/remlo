...
// Renamed from IIdentityRegistry to IIdentityRegistryForValidation to avoid
// a Solc collision with ReputationRegistryUpgradeable's inline interface of
// the same name (different shape, same name → duplicate-identifier error
// when both are imported by a single translation unit, e.g. DeployERC8004).
// Upstream's convention of inlining interfaces per-file is fine at contract
// scope but breaks at script scope; single-file rename is the minimal fix.
...
contract ValidationRegistryUpgradeable is OwnableUpgradeable, UUPSUpgradeable {
    ...
    // Upstream used `reinitializer(2) onlyOwner` — upgrade path only. See
    // IdentityRegistryUpgradeable.initialize for drift rationale.
