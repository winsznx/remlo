...
For existing Supabase projects created before the on-chain identity fix, run the SQL in `scripts/migrations/20260323_add_employer_admin_wallet.sql` and backfill each employer row with the Tempo or Privy wallet that actually funds `PayrollTreasury`.
